...
exports.submitOfflineDrills = catchAsyncError(async (req, res, next) => {
    ...
    if (!Array.isArray(formValues) || formValues.length === 0) {
        return res.status(400).json({ success: false, message: 'Form values are required' });
    }
        ...
        const updatePromises = formValues.map(field => {
            ...
            return OfflineDrill.updateOne(
                ...
                {
                    $set: {
                        'sessions.$[session].drills.$[drill].form.$[formField].value': field.value,
        ...
        if (modifiedCount === 0) {
            ...
            return res.status(404).json({ success: false, message: 'No matching documents found or no changes detected' });
        }
